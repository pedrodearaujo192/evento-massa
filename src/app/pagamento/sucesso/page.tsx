...
function SucessoContent() {
  ...
  useEffect(() => {
    if (!orderId) {
      setError("Não conseguimos localizar o identificador do seu pedido.");
    ...
    async function finalizeOrder() {
      ...
      try {
        ...
        if (!orderSnap.exists()) {
          setError('Registro do pedido não encontrado.');
          setIsProcessing(false);
          return;
        }
        ...
        if (orderData.items && Array.isArray(orderData.items)) {
          for (const item of orderData.items) {
            ...
            try {
              ...
              console.warn("Lote não encontrado:", item.id);
            }
          }
        }
        ...
        setError("Erro ao gerar ingressos digitais.");
        setIsProcessing(false);
      }
    }
        ...
        <main className="flex-1 flex items-center justify-center p-4">
          <Card className="max-w-md w-full text-center p-12 space-y-6 shadow-2xl border-none">
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground">Estamos gerando seus ingressos. Quase lá!</p>
            </div>
          </Card>
        </main>
      </div>
